...
/*   Updated: 2024/10/22 09:18:32 by sel-jett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_textures(t_game *m, char *av)
{
	...
	while (m->ln)
	{
		if (path_of_txr_nm(m->ln, "NO") >= 0)
			...
			m->ciel_color = ft_ciel(pt_txr(m, m->ln + 1 + \
			pt_ciel(m->ln, "C")), m->fd2);
		...
		(1) && (ft_return_fd(m->ln, m->fd2), m->ln = get_next_line(m->fd2), 0);
	}
